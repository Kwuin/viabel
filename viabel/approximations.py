...
class MFGaussian(ApproximationFamily):
    ...
    #added by Jingkun for scale
    def mean_and_scale(self, var_param):
        param_dict = self._pattern.fold(var_param)
        return param_dict['mu'], np.exp(param_dict['log_sigma'])
    ...
    # added by Jingkun return the gradient of g(x)

    def path_g(self, var_param, sample):
        param_dict = self._pattern.fold(var_param)
        return param_dict['mu'] + np.exp(param_dict['log_sigma']) * sample

    def grad_path(self, var_param):
        param_dict = self._pattern.fold(var_param)
        return np.exp(param_dict['log_sigma'])
